...
class EmbarcaderoPostLaunchMigrator implements RegisterCommandInterface {
	...
	public static function register_commands(): void {
		...
		WP_CLI::add_command(
			'newspack-content-migrator embarcadero-fix-or-delete-1969-posts',
			self::get_command_closure( 'cmd_embarcadero_fix_or_delete_1969_posts' ),
			[
				'shortdesc' => 'Fix or delete 1969 posts.',
				'synopsis'  => [
					[
						'type'        => 'assoc',
						'name'        => 'unpublish-1969-posts-csv-file-path',
						'description' => 'Path to the CSV file containing the posts to unpublish. The file is generated by the "newspack-content-migrator embarcadero-unpublish-1969-posts" command.',
						'optional'    => false,
						'repeating'   => false,
					],
				],
	...
	/**
	 * Unpublish 1969 posts.
	 * Callable for "newspack-content-migrator embarcadero-fix-or-delete-1969-posts".
	 *
	 * @param array $args       Positional arguments.
	 * @param array $assoc_args Associative arguments.
	 */
	public function cmd_embarcadero_fix_or_delete_1969_posts( $args, $assoc_args ) {
		global $wpdb;

		$unpublish_1969_posts_csv_file_path = $assoc_args['unpublish-1969-posts-csv-file-path'];

		$log_deleted                = 'embarcadero_fix_or_delete_1969_posts.log';
		$log_deleted_posts_csv_file = 'embarcadero_fix_or_delete_1969_posts.csv';

		$unpublish_print_stories = $this->get_data_from_csv_or_tsv( $unpublish_1969_posts_csv_file_path );

		$csv = fopen( $log_deleted_posts_csv_file, 'a' );
		// Add headers only if the file is empty.
		if ( filesize( $log_deleted_posts_csv_file ) === 0 ) {
			fputcsv( $csv, [ 'original_topic_id', 'wp_post_id', 'original_issue_id_is_0', 'original_non_existing_issue_id', 'original_seo_link', 'action' ] );
		}

		foreach ( $unpublish_print_stories as $unpublish_print_story ) {
			if ( empty( $unpublish_print_story['original_seo_link'] ) ) {
				// Delete the post if original_seo_link is empty.
				wp_delete_post( $unpublish_print_story['wp_post_id'], true );
				fputcsv( $csv, [ $unpublish_print_story['original_topic_id'], $unpublish_print_story['wp_post_id'], $unpublish_print_story['original_id_is_0'], $unpublish_print_story['original_non_existing_issue_id'], $unpublish_print_story['original_seo_link'], 'deleted' ] );
			} elseif ( str_starts_with( $unpublish_print_story['original_seo_link'], '1969' ) ) {
				// Delete the post if the original_seo_link starts with 1969.
				wp_delete_post( $unpublish_print_story['wp_post_id'], true );
				fputcsv( $csv, [ $unpublish_print_story['original_topic_id'], $unpublish_print_story['wp_post_id'], $unpublish_print_story['original_id_is_0'], $unpublish_print_story['original_non_existing_issue_id'], $unpublish_print_story['original_seo_link'], 'deleted' ] );
			} else {
				// Fix the publish date for the post and publish it.
				$seo_link = explode( '/', $unpublish_print_story['original_seo_link'] );
				$year     = $seo_link[0];
				$month    = $seo_link[1];
				$day      = $seo_link[2];

				$created_date     = date( 'Y-m-d H:i:s', strtotime( $year . '-' . $month . '-' . $day ) );
				...
				$wpdb->update(
					...
					[
						...
						'post_date_gmt'     => $created_date_gmt,
						'post_modified'     => $created_date,
						'post_modified_gmt' => $created_date_gmt,
						'post_status'       => 'publish',
					],
					[ 'ID' => $unpublish_print_story['wp_post_id'] ]
				);

				fputcsv( $csv, [ $unpublish_print_story['original_topic_id'], $unpublish_print_story['wp_post_id'], $unpublish_print_story['original_id_is_0'], $unpublish_print_story['original_non_existing_issue_id'], $unpublish_print_story['original_seo_link'], 'updated' ] );
			}
		}
